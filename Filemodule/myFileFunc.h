...
typedef struct
{
    ...
    /*for SIM 2*/
    BYTE szPriTxnIP_2[30+1];
    BYTE szSecTxnIP_2[30+1] ;
    BYTE szPriSettleIP_2[30+1];									//15
    BYTE szSecSettleIP_2[30+1];
    int inPriTxnPort_2;
    int inSecTxnPort_2;
    int inPriSettlePort_2;
    int inSecSettlePort_2;
} STRUCT_CPT;
...
typedef struct
{
	...
	BOOL    fQPS;
	BOOL    fQPSCustCopy;
	BYTE	szQPSUSDLimit[12+1];
	BYTE	szQPSLimit[12+1];

	BOOL    fVEPS;
	BOOL    fVEPSCustCopy;
	BYTE	szVEPSUSDLimit[12+1];
	BYTE	szVEPSLimit[12+1];
    ...
    char    chNextSettleTime[14+1];
    BOOL    fMustAutoSettle;
    char    szAutoSettleIntervalHour[4+1];
    int     inExchangeRate;
    int     inECRPrintMode;
    int     inDupReceipt;
    BOOL    fEnableAmountIdle;
    char    szMerchantName[50];
    char    szMerchantAddress[100];
    BYTE szApplicationPW[PWD_LEN+1];
    BOOL fEnableIPPMenu;
    BOOL fEnableAlipayMenu;
    int inRcptVia;	
    BOOL fEnablePreAuthMenu;
	BOOL fEnableUPISignONMenu;
	BOOL fUploadReceiptIdle;
	int inIdleTimeout;
	BOOL fSendTCBeforeSettle;
    USHORT usTMSRemotePort2;
    BYTE szTMSRemoteIP2[30+1];
	BOOL fUploadReceiptonSettle;
    BOOL fEnableDiscountMenu;	
	BOOL fDisplayIDLEOption;
    BOOL fEnableRefundMenu;
	BOOL fOKDollarPreConnect;
...
typedef struct
{
    ...
    BOOL fBTR; /*Balance/Topup/Reload flag*/
    int inBTR; /*Balance/Topup/Reload value*/
	BOOL 	fMagSwipeEnable;
	BOOL 	fPreAuthAllowed;
	BOOL 	fRefundAllowed;
	BOOL 	fCTLSAllowed;
	BOOL 	fAddRefRRNAPP;
...
typedef struct
{
	...
	USHORT inDeleteREV;
    USHORT inNumAdv;
	BOOL 	fTPDUOnOff; // MPU NH migration	
...
typedef struct
{
    ...
    BOOL fConfirmQRCode;
} STRUCT_FLG;/*Flag Table*/


// For CBPay
typedef struct
{
	BYTE    szGenerateURL[250+1];
	BYTE    szTranStatusURL[250+1];
	BYTE    szVoidURL[250+1];
	BYTE    szCancelURL[250+1];
	
	BYTE    szAuthenToken[300+1];
	BYTE    szSubMerId[20+1];
	
	int	inQP_CurlTimeout;
	int	inQP_CurlRetries;
...
{
	BYTE    szTranStatusURL[250+1];
	BYTE    szTranStatusSecURL[250+1];	
	BYTE    szVoidURL[250+1];
	BYTE    szVoidSecURL[250+1];
	
	BYTE    szTranStatusURLSimTwo[300+1];
	BYTE    szTranStatusSecURLSimTwo[300+1];
	BYTE    szVoidURLSimTwo[300+1];
	BYTE    szVoidSecURLSimTwo[300+1];
	
	int		inQP_CurlTimeout;
	int		inQP_CurlRetries;

	BOOL	fOKDollarSimOne;
...
STRUCT_CBPAY	 strCBPay;
STRUCT_CBOKD	 strCBOKD;
...
typedef struct{
    ...
    int inInstallmentPlan;
    char szSchemeID[25+1];
} STRUCT_IPS;
