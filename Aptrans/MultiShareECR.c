...
USHORT usCTOSS_ECRInitialize(void)
{
	...
	if(d_OK != usResult)
	 	 vdDebug_LogPrintf("**ERRR  ECRInitialize**");
...
int inCTOS_ECR_ECHO(void){
    ...
    vdCTOS_SetTransType(ECR_ECHO);
    
    CTOS_LCDTClearDisplay();
    vdDispTitleString("ECR ECHO");
    
    setLCDPrint(8, DISPLAY_POSITION_LEFT, "ECHO TEST");
...
int inCTOS_ECR_RETRIEVAL(void){
    ...
    vdCTOS_TxnsBeginInit();
    CTOS_LCDTClearDisplay();
    vdDispTitleString("TXN RETRIEVAL");
