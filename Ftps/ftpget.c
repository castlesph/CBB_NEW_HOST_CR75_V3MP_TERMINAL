...
int inFtpGet(unsigned char *uszFileName, unsigned char *uszFtpURL, unsigned char *uszFtpUserName, unsigned char *uszFtpPassword)
{
	CURL *curl;
	CURLcode res;
...
//	strCPT.inCommunicationMode = GPRS_MODE;
	...
	vdDebug_LogPrintf("filename=[%s]", ftpfile.filename);
	...
	vdDebug_LogPrintf("uszCURLURL=[%s]", uszCURLURL);
  ...
  vdDebug_LogPrintf("inFtpGet END");
