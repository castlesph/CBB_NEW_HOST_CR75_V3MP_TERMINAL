...
pthread_t K_fdCommUIThread = 0;
pthread_t K_fdPrntUIThread = 0;
...
int inSetTextModeEx(BYTE *inbuf, USHORT inlen)
{
	...
	if (strcmp(inbuf, "tahoma.ttf") == 0)	
	{
		CTOS_LCDTTFSwichDisplayMode(d_TTF_MODE_VIETNAM);

		//CTOS_PrinterTTFSelect(d_TTF_MODE_VIETNAM, 0);
		//inCTOSS_SetERMFontType(d_TTF_MODE_VIETNAM, 0);
		CTOS_PrinterTTFSelect(inbuf, 0);
		inCTOSS_SetERMFontType((char *)inbuf, 0);
		CTOS_PrinterTTFSwichDisplayMode(d_TTF_MODE_VIETNAM);
	}
	else if (strcmp(inbuf, THEMESTYPE_7_FONT) == 0)	
	{
		//CTOS_LCDTTFSwichDisplayMode(d_TTF_MODE_VIETNAM);
...
{   
	...
	{
		//CTOS_LCDForeGndColor(RGB(34, 93, 171));// you may need adjust the color menu item bitmap
		CTOS_LCDForeGndColor(RGB(39, 98, 176));   //0x00 B0 62 27
		CTOS_LCDBackGndColor(RGB(255, 255, 255)); // 0x00 FF FF FF
	}
...
void vdDisplayCommBmpEx(void *arg)
{
...
void vdThreadDisplayCommBmpEx(unsigned int x,unsigned int y, const char *path1, const char *path2, const char *path3, const char *path4, const char *path5)
{
    int inStatus;
    int stack_size;
    void *sp = NULL;
    ...
    pthread_create(&K_fdCommUIThread, NULL, (void *)vdDisplayCommBmpEx, (void*)NULL);
...
void vdDisplayPrintBmpEx(void *arg)
{
	pthread_detach(pthread_self());

    while(1)
    {
        if(0 == inGetPrintThreadStatus())
        {
            vdDebug_LogPrintf("vdDisplayPrintBmpEx pthread_exit");
            pthread_exit(NULL);
        }   
        else
        {                 
            if (StAnimatePrintBmp.path1!=NULL && 1 == inGetPrintThreadStatus())
            {
                //vdDebug_LogPrintf("pvdDisplayCommBmp path1");
                displaybmpEx(StAnimatePrintBmp.x, StAnimatePrintBmp.y, StAnimatePrintBmp.path1);
                CTOS_Delay(100);
            ...
            if (StAnimatePrintBmp.path2!=NULL && 1 == inGetPrintThreadStatus())
            {
                //vdDebug_LogPrintf("pvdDisplayCommBmp path2");
                displaybmpEx(StAnimatePrintBmp.x, StAnimatePrintBmp.y, StAnimatePrintBmp.path2);
                CTOS_Delay(100);
            ...
            if (StAnimatePrintBmp.path3!=NULL && 1 == inGetPrintThreadStatus())
            {
                //vdDebug_LogPrintf("pvdDisplayCommBmp path3");
                displaybmpEx(StAnimatePrintBmp.x, StAnimatePrintBmp.y, StAnimatePrintBmp.path3);
                CTOS_Delay(100);
            ...
            if (StAnimatePrintBmp.path4!=NULL && 1 == inGetPrintThreadStatus())
            {
                //vdDebug_LogPrintf("pvdDisplayCommBmp path4");
                displaybmpEx(StAnimatePrintBmp.x, StAnimatePrintBmp.y, StAnimatePrintBmp.path4);
                CTOS_Delay(100);
            ...
            if (StAnimatePrintBmp.path5!=NULL && 1 == inGetPrintThreadStatus())
            {
                //vdDebug_LogPrintf("pvdDisplayCommBmp path5");
                displaybmpEx(StAnimatePrintBmp.x, StAnimatePrintBmp.y, StAnimatePrintBmp.path5);
                CTOS_Delay(100);
...
void vdThreadDisplayPrintBmpEx(unsigned int x,unsigned int y, const char *path1, const char *path2, const char *path3, const char *path4, const char *path5)
{
    int inStatus;
    int stack_size;
    void *sp = NULL;
    ...
    pthread_create(&K_fdPrntUIThread, NULL, (void *)vdDisplayPrintBmpEx, (void*)NULL);
